...
class SignUp {
    ...
    // you can certainly call cy.fixture once and reuse the loaded fixture data across different functions in a Cypress Page Object Model (POM).

    constructor()
    {

        cy.fixture('signup_creds').as('userData').then((userData) => {
    ...
    check_signup_fields_validations() {
        ...
        cy.fixture("signup_creds").then((creds) => {
            ...
            cy.get(this.confirm_pw_field).type("qwe").blur()
            ...
            cy.get(this.i_agree_checkbox).check()
            cy.get(this.register_btn).should('not.have.attr', 'disabled');
